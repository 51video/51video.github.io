    ...
    return function() {
        ...
        function() {
            ...
            _createClass(Iti, [ {
                ...
                key: '_appendSearch',
                value: function _appendSearch() {
                    var that = this;
                    var tmp = '<div style="position: absolute; top: 0; left: 0; width: 100%; padding: 10px; z-index: 2000;">' +
                                '<input id="telSearchInput" type="text" placeholder="搜索国家或地区" class="form-control" autocomplete="off">' +
                               '</div>';
                    $(this.countryList).prepend(tmp);
                    this.countryList.style.paddingTop = '58px';
                    $(this.dropdown).on('click', '#telSearchInput', function(event) {
                        $(this).focus();
                        event.stopPropagation();
                    });
                    $(this.dropdown).on('focus', '#telSearchInput', function(event) {
                        event.stopPropagation();
                    });
                    $(this.dropdown).on('keyup', '#telSearchInput', function(event) {
                        var query = $('#telSearchInput').val();
                        var code = query.charCodeAt(0);
                        if (code > 48 && code <= 57) {
                            var queryNum = query[0];
                            $('#country-listbox .country').each(function() {
                                $(this).css('display', String($(this).data('dial-code')).indexOf(queryNum) != 0 ? 'none' : 'block');
                            });
                            $('#country-listbox').scrollTop(0);
                            return true;
                        }
                        $('#country-listbox').find('li').css('display', 'block');
                        that._searchForCountry(query.toLowerCase());
                    });
                    $(this.countryList).on('scroll', function() {
                        $('#telSearchInput').parent().css('top', this.scrollTop + 'px');
                    });
                }
            }, {
                ...
                value: function _showDropdown() {
                    ...
                    this._setDropdownPosition(true);

                    // close menu on window scroll
                    this._handleWindowScroll = function() {
                        if (that.isMobile) {
                            that._setDropdownPosition(false);
                        } else {
                            return that._closeDropdown();
                        }
                    };
                ...
                value: function _setDropdownPosition(tag) {
                    var _this8 = this;
                    if (tag && this.options.dropdownContainer) {
                    ...
                    if (true) {
                        ...
                        if (this.options.dropdownContainer) {
                            ...
                            var extraTop = this.telInput.offsetHeight;
                            // calculate placement
                ...
                value: function _bindDropdownListeners() {
                    ...
                    this._handleClickOffToClose = function() {
                        ...
                        isOpening = false;
                        if (_this9.options.search) {
                            $('#telSearchInput').parent().css('top', '0');
                        }
                    };
